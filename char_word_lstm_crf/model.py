...
class NERModel(object):
        ...
        with tf.variable_scope("words"):
            if self.embeddings:
                ...
                _word_embeddings = tf.get_variable(name="_word_embeddings", dtype=tf.float32, 
                                shape=[self.config.nwords,self.config.dim],
                                trainable=self.config.train_embeddings)
        ...
        for i, (words, labels) in enumerate(minibatches(train, self.config.batch_size)):
            ...
            if i % 2000 ==0:
                ...
                if f1>self.best_score:
                    print 'New best, save model',acc,f1
    ...
    def train(self, train, dev, tags):
        ...
        nepoch_no_imprv = 0
        with tf.Session(config=self.config.session_conf) as sess:
            sess.run(self.init)
            # tensorboard
    ...
    def interactive_shell(self, tags, processing_word, test):
