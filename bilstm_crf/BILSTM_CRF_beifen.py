...
class BILSTM_CRF(object):
    ...
    def __init__(self, num_chars, num_classes, num_steps=200, num_epochs=100000000, embedding_matrix=None, is_training=True, is_crf=True, weight=False):
        # Parameter
        self.max_f1 = 0
        self.last_f = 1
...
#        self.inputs_emb = tf.split(0, self.num_steps, self.inputs_emb)
    ...
    def train(self, sess, save_file, X_train_source, y_train_source, X_val_source, y_val_source,
        ...
        count_source = 0
        count_target = 0
        print "the source probability is %f"%source_probability
...
#                y_train = y_train_target[sh_index]
                ...
                self.train_an_iteration(sess, save_file, X_train, y_train, X_val_target, y_val_target, char2id, id2char, label2id, id2label, summary_writer_train_source, summary_writer_train_target, summary_writer_val_source, summary_writer_val_target, flag="target", is_summary=(count_target%250==0), is_validation=(count_target%10==0), X_test_source=X_test_source, y_test_source=y_test_source)
                count_target += 1
...
##      return results
            ...
            length, max_scores, max_scores_pre = sess.run([self.length, self.max_scores_source if flag == "source" else self.max_scores_target, self.max_scores_pre_source if flag == "source" else self.max_scores_pre_target], feed_dict={self.inputs:predict_data})
            predicts = self.viterbi(max_scores, max_scores_pre, length, batch_size)
...
#                    tmp.append(label2id[line.strip().split()[1]])
        ...
        print "presicion_loc:%f, recall_loc:%f, f_loc:%f, presicion_org:%f, recall_org:%f, f_org:%f, presicion_per:%f, recall_per:%f, f_per:%f"%(presicion_loc, recall_loc, f_loc, presicion_org, recall_org, f_org, presicion_per, recall_per, f_per)
